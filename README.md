...
In the original skeleton file we both spent a lot of time attempting to re-do the Sylow.lean file without much understanding of how lean works and the paticulars of type classes and finite objects. After realising that the majority of what we had done to build the skeleton was fully incorrect, we shifted our goal to focus purely on the corollaries.
...
Tom: I orchestrated a lot of the technical requirements for the project, managing the git repository and setting up cloud infrastructure for remote working on gitpod. This was originally very error prone, paticularly with configuring dependencies for the gitpod cloud workspaces, where lean had to be reinstalled each time, and I experimented with bash scripts to run the necessary commands on launch. Things eventually worked consistently. I instructed my group on how to use git, and the basic concepts of branches, pushing and pulling etc. It took some time to get used to it, paticularly when merge conflicts arose, the resolving of which was cumbersome over the command line. I experimented with managing several branches, but eventually the manual overhead of having to pull request in each branch to master regularly, and the growing desynchronisation between branches, led me to return to the original approach of us all working on the master branch.

I commented hundreds of lines of mathlib code, and discussed what I found with my group to help further our understanding on how Mathlib efficiently implements Group Theory in the context of lean, explaining things like how type coercion was used to convert Sylow groups to subgroups, and how the algebraic structures like monoids and semigroups defined under groups. Some of the set theoretic concepts in Mathlib took some research to understand, and there were several group theoretic results I had not encountered before. I also streamlined and commented on lots of our code, to help make our code more legible and concise. I also wrote several drafts of the README.md file, including a lot of typesetted maths in Latex, and ensured references to our source material on group theory were accurate. i helped clean up the repository, and ensure unnecessary files were deleted as they became unnecessary.
