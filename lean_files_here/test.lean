...
-- semiring is ring without subtraction
-- additive semigroup (no additvie inverse = subtraction)
-- multiplicative semigroup (no multiplicative inverse = division)
...
-- variable declaration, r in type, means a set, semiring R is
-- R is a semiring, informing lean that if two elements in R need to be added or multiplied
-- tells lean they can use the semiring results in proofs, we also have an element r in R
...
#check R[X]
#guard_msgs(drop error) in
#check R[Y] -- polynomials have to be in x unless you redefine all the notation
...
example : natDegree (X + 1 : ℤ[X]) = 1 := by
  rw [natDegree_add_eq_left_of_natDegree_lt] <;>
  simp -- simp runs thru algo to try and simplify
