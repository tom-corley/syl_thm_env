...
-- Took this theorem from MatMathlib.GroupTheory.Cosethlib.GroupTheory.OrderOfElement.
...
theorem blah [Finite (Sylow p G)] (P : Sylow p G) (g : G)
(h : Unique (Sylow p G)) : ↑P = ↑(g • P) := by
   apply?


theorem normal_of_uniques [Finite (Sylow p G)] (P : Sylow p G)
(h : ∀ (g : G), ↑P = ↑(g • P) ) : (P : Subgroup G).Normal := by
  refine Sylow.normal_of_normalizerCondition ?hnc P
...
-- Index |G:Q| is equal to p
  have h3 : Fintype.card (G ⧸ Q) = p := by sorry
